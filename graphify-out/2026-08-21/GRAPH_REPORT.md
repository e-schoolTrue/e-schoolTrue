# Graph Report - e-schoolTrue  (2026-08-21)

## Corpus Check
- 304 files · ~314,997 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3329 nodes · 5568 edges · 250 communities (183 shown, 67 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 176 edges (avg confidence: 0.74)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5dbc288f`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- license-cli.ts
- data-source.ts
- CloudSyncService
- PrintAndModelView.vue
- GradeEntity
- FileEntity
- ProfessorEntity
- PlanningConfigView.vue
- compilerOptions
- ReportTemplateThree.vue
- note-config-service.ts
- professor.ts
- PaymentManagementView.vue
- PayementConfigurationView.vue
- AbsenceProfView.vue
- CentralizedGradesView.vue
- .populateEntitySyncMetas
- PaymentService
- ReportCardView.vue
- SchoolInfoView.vue
- ResultType
- AbsenceForm.vue
- shared.ts
- LicenseStatusView.vue
- AbsenceEntity
- compilerOptions
- PaymentDialog.vue
- .hideLoader
- HomeworkView.vue
- AnnualPVView.vue
- StudentService
- gradeEntryService.ts
- ClassRoomView.vue
- StudentListView.vue
- payment.ts
- VacationView.vue
- AbsenceStudentView.vue
- VacationView.vue
- DocumentsView.vue
- reportCardService.ts
- StudentDetailsView.vue
- centralized-grade-sheet.vue
- ProfessorPaymentView.vue
- CalculatedGradeEntity
- NoteConfigView.vue
- student-table.vue
- StudentCardView.vue
- YearRepartitionService
- BulletinTemplateTwo.vue
- YearRepartitionView.vue
- BulletinTemplateOne.vue
- course-table.vue
- GradeView.vue
- professor-table.vue
- student-file.vue
- StudentPlanningView.vue
- centralizedPdfService.ts
- electron-env.d.ts
- PaymentHistory.vue
- student-form.vue
- electron-env.d.ts
- index.ts
- YearRepartitionView.vue
- main.ts
- grade.ts
- ProfessorPaymentDialog.vue
- useCurrency.ts
- SyncView.vue
- GradeConfigEntity
- AuthService
- PaymentDaily.vue
- professor-form.vue
- GeneralInfoView.vue
- events.ts
- course-form.vue
- grade-table.vue
- YearRepartionForm.vue
- CourseView.vue
- DashboardView.vue
- PayementConfigurationView.vue
- loadStudents
- Guide de la fonctionnalité de Mise à Jour Automatique
- PreferenceEntity
- SimulationCalculator.vue
- ProfessorDetailsView.vue
- GradeService
- BulletinConfigDialog.vue
- CurrencyDisplay.vue
- PaymentHistoryMini.vue
- course.ts
- GradeView.vue
- LicenseView.vue
- ScheduleConfigEntity
- .getInstance
- ScheduleService
- license.spec.ts
- preload.ts
- card.ts
- ColorSchemeSelector.vue
- course-details.vue
- phone-verification-list.vue
- TeachingAssignment.vue
- student-filter.vue
- themeStore.spec.ts
- ClassRoomEntity
- SchoolEntity
- gradeService.ts
- getAppreciation
- UpdateNotifier.vue
- TrancheConfigDetails.vue
- NewStudentView.vue
- student-update.vue
- SupervisorInfoView.vue
- handlePrint
- dependencies
- devDependencies
- branch-form.vue
- create-account.vue
- login-form.vue
- ForgotPasswordView.vue
- course-group-form.vue
- GradeCalculationDetail.vue
- ./sections/Attachments.vue
- SyncHistory.vue
- ApparenceView.spec.ts
- ConfigurationWizard.vue
- LanguageSettingView.vue
- settingsCommand.ts
- 🔑 License Management
- CardTemplateBase.vue
- SyncSettings.vue
- PlanningScheduleConfig.vue
- User
- License
- scripts
- App.vue
- CardTemplateThree.vue
- EditableCell.vue
- student-selector.vue
- html2pdf.d.ts
- DataLocationView.vue
- CentralizedPdfService
- xlsx
- CardTemplateOne.vue
- CardTemplateTwo.vue
- dashbord-menu.vue
- LicenseChecker.vue
- LoginView.vue
- ApparenceView.vue
- 001_schema_provisioning.sql
- message-customizer.vue
- MensualityConfigTable.vue
- TranchConfigTable.vue
- loadStudentGrades
- package.json
- AbsenceList.vue
- ./sections/SchoolInfo.vue
- electron.d.ts
- preference.ts
- SyncAPI
- vacation.ts
- activateLicense
- WizardViewBase.vue
- handleAuthSuccess
- components.d.ts
- utils.ts
- PdfService
- env.spec.ts
- Issue with *Red Hat Dependency Analytics Extension*
- selectOptions.ts
- dashboard.ts
- gradeConfig.ts
- homework.ts
- scholarship.ts
- vue-tsc
- CardFlipController.vue
- calculateCourseAverage
- cardWorker.ts
- better-sqlite3
- bufferutil
- chart.js
- csv-parser
- date-fns
- electron
- electron-builder
- electron-print-preview
- electron-rebuild
- electron-store
- supabase.ts
- electron-updater
- handlebars
- html2pdf.js
- @iconify-json/bi
- @iconify-json/carbon
- @iconify-json/ep
- @iconify-json/mdi
- @iconify-json/vaadin
- @iconify/vue
- jsdom
- jspdf-autotable
- @mdi/js
- @noble/ed25519
- @noble/hashes
- node-machine-id
- ApparenceView.spec.ts
- @supabase/supabase-js
- typeorm
- supabase.ts
- @types/jsdom
- pinia
- CascadeDelete
- qrcode.vue
- tsx
- ./sections/SchoolInfo.vue
- @types/jspdf
- typescript
- unplugin-auto-import
- unplugin-element-plus
- unplugin-icons
- unplugin-vue-components
- vite
- vite-plugin-electron
- vite-plugin-electron-renderer
- @vitejs/plugin-vue
- vitest
- vue
- vue-chartjs
- @vue/devtools
- vue-html-to-paper
- vue-router
- @vue/test-utils
- OnboardingView.vue
- shims-vue.d.ts
- document.ts
- ValidateAccountView.vue
- grade.ts
- @supabase/supabase-js
- reflect-metadata
- @pinia/testing

## God Nodes (most connected - your core abstractions)
1. `GradeEntity` - 66 edges
2. `CourseEntity` - 57 edges
3. `StudentEntity` - 56 edges
4. `ProfessorEntity` - 49 edges
5. `CloudSyncService` - 37 edges
6. `PaymentService` - 37 edges
7. `ResultType` - 35 edges
8. `AppDataSource` - 33 edges
9. `AbsenceEntity` - 30 edges
10. `FileEntity` - 29 edges

## Surprising Connections (you probably didn't know these)
- `exportToPdf()` --references--> `jspdf`  [EXTRACTED]
  src/views/student/PaymentManagementView.vue → package.json
- `exportToExcel()` --references--> `xlsx`  [EXTRACTED]
  src/components/payment/PaymentHistory.vue → package.json
- `handleExport()` --references--> `xlsx`  [EXTRACTED]
  src/components/professor/professor-table.vue → package.json
- `handleFileSelect()` --references--> `xlsx`  [EXTRACTED]
  src/components/student/student-file.vue → package.json
- `handleExport()` --references--> `xlsx`  [EXTRACTED]
  src/components/student/student-table.vue → package.json

## Import Cycles
- None detected.

## Communities (250 total, 67 thin omitted)

### Community 0 - "license-cli.ts"
Cohesion: 0.06
Nodes (65): assertCommonFields(), base64UrlDecode(), base64UrlEncode(), buildMachineFingerprint(), daysRemaining(), decodePayload(), deriveSubPublicKey(), deriveSubSignKey() (+57 more)

### Community 1 - "data-source.ts"
Cohesion: 0.06
Nodes (34): PaymentEntity, Column, CreateDateColumn, DeleteDateColumn, Entity, JoinColumn, ManyToOne, PrimaryGeneratedColumn (+26 more)

### Community 2 - "CloudSyncService"
Cohesion: 0.10
Nodes (8): BackupEntity, Column, CreateDateColumn, Entity, PrimaryGeneratedColumn, UpdateDateColumn, CloudSyncService, isValidUuid()

### Community 3 - "PrintAndModelView.vue"
Cohesion: 0.03
Nodes (54): AbsenceData, absencesByStudent, classAverage, classes, colorOptions, countryHeaderMap, courses, currentTemplateComponent (+46 more)

### Community 4 - "GradeEntity"
Cohesion: 0.13
Nodes (18): CourseEntity, ObservationEntity, Column, CreateDateColumn, DeleteDateColumn, Entity, JoinColumn, JoinTable (+10 more)

### Community 5 - "FileEntity"
Cohesion: 0.13
Nodes (14): activeStep, emit, form, formRef, goToStep(), handleSubmit(), loading, nextStep() (+6 more)

### Community 6 - "ProfessorEntity"
Cohesion: 0.09
Nodes (32): DiplomaEntity, QualificationEntity, Column, CreateDateColumn, DeleteDateColumn, Entity, OneToMany, PrimaryGeneratedColumn (+24 more)

### Community 7 - "PlanningConfigView.vue"
Cohesion: 0.06
Nodes (25): availableTeachingItems, Class, classes, conflicts, Course, courseColors, days, defaultSlots (+17 more)

### Community 8 - "compilerOptions"
Cohesion: 0.05
Nodes (40): ./*, auto-imports.d.ts, DOM, DOM.Iterable, ./node_modules/@types, node_modules/unplugin-auto-import/auto-imports.d.ts, src/**/*.ts, src/**/*.tsx (+32 more)

### Community 9 - "ReportTemplateThree.vue"
Cohesion: 0.10
Nodes (12): Props, getCountryInfo, Props, reportTemplates, ConductData, GenerateReportCardInput, GenerateReportCardsInput, GradeData (+4 more)

### Community 10 - "note-config-service.ts"
Cohesion: 0.12
Nodes (25): CalculationStrategy, EvaluationCategoryEntity, GradingConfigEntity, Column, CreateDateColumn, Entity, Index, ManyToOne (+17 more)

### Community 11 - "professor.ts"
Cohesion: 0.11
Nodes (26): Teaching, IProfessorData, IProfessorDetails, IProfessorFile, IProfessorServiceParams, IProfessorStatistics, ROLE, CivilityType (+18 more)

### Community 12 - "PaymentManagementView.vue"
Cohesion: 0.05
Nodes (20): classConfigs, currentPage, dailyReportVisible, Filters, Grade, grades, historyDialogVisible, loading (+12 more)

### Community 13 - "PayementConfigurationView.vue"
Cohesion: 0.07
Nodes (28): activeTab, calculatedMonthlyAmount, { currency }, currentCustomConfig, currentPaymentConfig, customPaymentConfigs, deleteCustomConfig(), Grade (+20 more)

### Community 14 - "AbsenceProfView.vue"
Cohesion: 0.06
Nodes (23): formatTimeSlotForDisplay(), AbsenceInfo, classes, defaultTimeSlots, filteredProfessorsSchedule, formatTimeSlot(), loading, pendingAbsence (+15 more)

### Community 15 - "CentralizedGradesView.vue"
Cohesion: 0.06
Nodes (29): CentralizedRanking, classAverage, classConfig, Course, courses, currentClass, currentPage, exportingExcel (+21 more)

### Community 16 - ".populateEntitySyncMetas"
Cohesion: 0.17
Nodes (11): Column, Entity, PrimaryGeneratedColumn, UserEntity, electronStore, getSchemaClient(), SchemaClient, schemaClients (+3 more)

### Community 17 - "PaymentService"
Cohesion: 0.13
Nodes (12): PaymentService, IInscriptionFee, IPaymentConfigData, IPaymentData, IPaymentFee, IPaymentServiceParams, IPaymentServiceResponse, IProfessorPaymentData (+4 more)

### Community 18 - "ReportCardView.vue"
Cohesion: 0.06
Nodes (26): calculationDetailRef, categories, Category, classes, configInfo, Course, courses, generalAverage (+18 more)

### Community 19 - "SchoolInfoView.vue"
Cohesion: 0.13
Nodes (14): countries, fileInput, foundationYears, hasChanges, initialSchoolInfo, isEditMode, isLoading, isSaving (+6 more)

### Community 20 - "ResultType"
Cohesion: 0.19
Nodes (12): Column, CreateDateColumn, Entity, PrimaryGeneratedColumn, UpdateDateColumn, YearRepartitionEntity, YearRepartitionService, PeriodConfiguration (+4 more)

### Community 21 - "AbsenceForm.vue"
Cohesion: 0.07
Nodes (24): Absence, AbsenceFormData, AbsenceSubmitData, absenceTypes, availableCourses, Course, dialogVisible, emit (+16 more)

### Community 22 - "shared.ts"
Cohesion: 0.13
Nodes (24): IAbsenceData, IAbsenceServiceParams, IAbsenceStatistics, IRecentAbsence, ResponseType, IProfessorServiceResponse, Gender, IAbsence (+16 more)

### Community 23 - "LicenseStatusView.vue"
Cohesion: 0.06
Nodes (25): ActivateResponse, activationType, daysRemaining, generatedLicenseCode, GenerateSubResponse, GetDetailsResponse, GetStatusResponse, isGenerating (+17 more)

### Community 24 - "AbsenceEntity"
Cohesion: 0.19
Nodes (3): AbsenceService, IAbsenceServiceParams, IAbsenceServiceResponse

### Community 25 - "compilerOptions"
Cohesion: 0.07
Nodes (29): dist, electron/**/*.ts, node_modules, compilerOptions, allowSyntheticDefaultImports, baseUrl, composite, declaration (+21 more)

### Community 26 - "PaymentDialog.vue"
Cohesion: 0.09
Nodes (28): { currency }, dialogTitle, dialogVisible, emit, form, formRef, getAdjustedTotalAmountDue(), getMaxAmountToPay (+20 more)

### Community 27 - ".hideLoader"
Cohesion: 0.17
Nodes (21): deleteClassRoom(), newClassRoom(), refreshClassRooms(), updateClassRoom(), fetchCourses(), handleAddToGroup(), handleDeleteCourse(), handleNewCourse() (+13 more)

### Community 28 - "HomeworkView.vue"
Cohesion: 0.07
Nodes (22): Course, courses, defaultMessage, deleteHomework(), dialogVisible, filteredHomework, form, Grade (+14 more)

### Community 29 - "AnnualPVView.vue"
Cohesion: 0.08
Nodes (24): AnnualDiscipline, AnnualDistinctions, AnnualStudentRecord, classAverage, classConfig, currentClass, currentPage, exportingPDF (+16 more)

### Community 30 - "StudentService"
Cohesion: 0.11
Nodes (17): StudentService, ResponseType, IAbsenceRelation, IFileRelation, IGradeRelation, IGradesCentralization, IPaymentRelation, IScholarshipRelation (+9 more)

### Community 31 - "gradeEntryService.ts"
Cohesion: 0.14
Nodes (14): currGrade, dialogVisible, ApiResponse, BranchEntity, BranchFormData, BranchResponse, ClassRoomResponse, GradeFormData (+6 more)

### Community 32 - "ClassRoomView.vue"
Cohesion: 0.08
Nodes (21): emits, filteredClassRooms, gradeDetailsRef, paginator, props, searchForm, dialogVisible, emits (+13 more)

### Community 33 - "StudentListView.vue"
Cohesion: 0.08
Nodes (24): loadLogo(), loadSchoolInfo(), logoPreview, schoolInfo, IFilterCriteria, filterCriteria, filteredStudents, getGradeName() (+16 more)

### Community 34 - "payment.ts"
Cohesion: 0.09
Nodes (22): CustomPaymentConfig, IPaymentAnnualConfig, IPaymentConfigData, IPaymentServiceParams, IPaymentServiceResponse, IScholarshipData, ITranchConfig, ITrancheEntry (+14 more)

### Community 35 - "VacationView.vue"
Cohesion: 0.09
Nodes (20): approveVacation(), closeDialog(), dateRange, deleteVacation(), dialogVisible, filteredVacations, form, formRef (+12 more)

### Community 36 - "AbsenceStudentView.vue"
Cohesion: 0.07
Nodes (19): Absence, absences, absenceTypes, currentDocument, currentPage, dateShortcuts, dialogVisible, filteredAbsences (+11 more)

### Community 37 - "VacationView.vue"
Cohesion: 0.08
Nodes (19): approveVacation(), dateRange, deleteVacation(), dialogVisible, filteredVacations, form, isEditing, loading (+11 more)

### Community 38 - "DocumentsView.vue"
Cohesion: 0.15
Nodes (9): activeTab, documentId, inscriptionContent, inscriptionRef, isSaving, logoPreview, schoolInfo, scolariteContent (+1 more)

### Community 39 - "reportCardService.ts"
Cohesion: 0.14
Nodes (16): ReportCardEntity, Column, CreateDateColumn, Entity, JoinColumn, ManyToOne, PrimaryGeneratedColumn, ReportCardService (+8 more)

### Community 40 - "StudentDetailsView.vue"
Cohesion: 0.09
Nodes (13): activeNames, CurrentDocument, dialogVisible, inscriptionContent, isGeneratingCertificate, isGeneratingInscription, photoUrl, printingType (+5 more)

### Community 41 - "centralized-grade-sheet.vue"
Cohesion: 0.08
Nodes (19): filteredRankings, filteredRankingsBase, filters, loading, paginator, props, rankings, searchQuery (+11 more)

### Community 42 - "ProfessorPaymentView.vue"
Cohesion: 0.08
Nodes (23): IProfessorPaymentData, exportToExcel(), filteredPayments, Filters, formatDate(), formatPaymentMethod(), formatPaymentType(), handleFilter() (+15 more)

### Community 43 - "CalculatedGradeEntity"
Cohesion: 0.09
Nodes (18): CalculatedGradeEntity, GradeEntryEntity, Column, CreateDateColumn, Entity, Index, JoinColumn, ManyToOne (+10 more)

### Community 44 - "NoteConfigView.vue"
Cohesion: 0.09
Nodes (17): CalculationStrategy, CategoryItem, classesList, ConfigForm, context, ExistingConfig, form, loadConfig() (+9 more)

### Community 45 - "student-table.vue"
Cohesion: 0.10
Nodes (24): boysCount, currentPage, emit, filteredStudents, getGradeName(), girlsCount, grades, handleClick() (+16 more)

### Community 46 - "StudentCardView.vue"
Cohesion: 0.09
Nodes (19): activeTab, filteredStudents, getCurrentTemplate, grades, handleExportPDF(), loading, previewContainer, previewStudent (+11 more)

### Community 47 - "YearRepartitionService"
Cohesion: 0.14
Nodes (14): Column, CreateDateColumn, DeleteDateColumn, Entity, ManyToOne, PrimaryGeneratedColumn, UpdateDateColumn, VacationEntity (+6 more)

### Community 48 - "BulletinTemplateTwo.vue"
Cohesion: 0.10
Nodes (18): AnnualDecision, computedAnnualAverage, containerStyle, countryData, countryHeaderMap, decisionLabels, decisionState, generalAverage (+10 more)

### Community 49 - "YearRepartitionView.vue"
Cohesion: 0.11
Nodes (17): academicYearEnd, academicYearStart, addPeriod(), datePickerOptions, emit, formatDateToString(), formRef, generatedSchoolYear (+9 more)

### Community 50 - "BulletinTemplateOne.vue"
Cohesion: 0.10
Nodes (17): AnnualDecision, computedAnnualAverage, containerStyle, countryData, countryHeaderMap, decisionLabels, decisionState, generalAverage (+9 more)

### Community 51 - "course-table.vue"
Cohesion: 0.12
Nodes (16): addNewSubModule(), courseDetailsRef, currentPage, currentParentCourseForSubModule, emit, filteredCourses, handleAddToGroup(), handleDelete() (+8 more)

### Community 52 - "GradeView.vue"
Cohesion: 0.11
Nodes (13): dialogVisible, emits, form, formRef, formRule, props, GradeCommand, grades (+5 more)

### Community 53 - "professor-table.vue"
Cohesion: 0.12
Nodes (16): currentPage, emit, filteredProfessors, getCivilityLabel(), getFamilySituationLabel(), handleDelete(), handleEdit(), handleExport() (+8 more)

### Community 54 - "student-file.vue"
Cohesion: 0.11
Nodes (16): availableGrades, columnMappings, displayFileName, displayFileSize, emit, excelData, fileHeaders, fileInput (+8 more)

### Community 55 - "StudentPlanningView.vue"
Cohesion: 0.11
Nodes (21): activeDays, classes, courseColors, DayItem, days, defaultStudentSlots, filters, getCellStyle() (+13 more)

### Community 56 - "centralizedPdfService.ts"
Cohesion: 0.25
Nodes (7): DocumentContentEntity, Column, CreateDateColumn, Entity, PrimaryGeneratedColumn, UpdateDateColumn, documentContentService

### Community 57 - "electron-env.d.ts"
Cohesion: 0.11
Nodes (7): DownloadProgress, ElectronAPI, IpcRenderer, NodeJS, ProcessEnv, UpdateInfo, Window

### Community 58 - "PaymentHistory.vue"
Cohesion: 0.13
Nodes (18): { currency }, formattedAmount, props, adjustedAnnualAmount, annualAmount, dialogVisible, emit, exportToExcel() (+10 more)

### Community 59 - "student-form.vue"
Cohesion: 0.12
Nodes (14): ./sections/ParentInfo.vue, ./sections/PersonalInfo.vue, Attachments, ClassItem, currentStep, emit, formData, ParentInfo (+6 more)

### Community 60 - "electron-env.d.ts"
Cohesion: 0.11
Nodes (7): DownloadProgress, ElectronAPI, IpcRenderer, NodeJS, ProcessEnv, UpdateInfo, Window

### Community 61 - "index.ts"
Cohesion: 0.15
Nodes (10): authRoutes, fileRoutes, ConfigurationWizard(), routes, onboardingRoutes, paymentRoutes, planningRoutes, professorRoutes (+2 more)

### Community 62 - "YearRepartitionView.vue"
Cohesion: 0.15
Nodes (13): PeriodConfiguration, YearRepartitionCreateInput, YearRepartitionResponse, YearRepartitionUpdateInput, closeModal(), currentRepartition, deleteRepartition(), fetchYearRepartitions() (+5 more)

### Community 63 - "main.ts"
Cohesion: 0.10
Nodes (26): BranchEntity, ClassRoomEntity, GradeEntity, Column, CreateDateColumn, DeleteDateColumn, Entity, JoinColumn (+18 more)

### Community 64 - "grade.ts"
Cohesion: 0.14
Nodes (17): InscriptionFeeEntity, PaymentAnnualConfigEntity, PaymentConfigEntity, TranchConfigEntity, TrancheEntryEntity, Column, Entity, JoinColumn (+9 more)

### Community 65 - "ProfessorPaymentDialog.vue"
Cohesion: 0.13
Nodes (15): calculateNetAmount(), dialogVisible, emit, form, formRef, handleClose(), handleSubmit(), loading (+7 more)

### Community 66 - "useCurrency.ts"
Cohesion: 0.13
Nodes (14): formatCurrency(), formatFCFA(), currency, currencyCode, absenceChartRef, createGradient(), { currency }, currentDate (+6 more)

### Community 67 - "SyncView.vue"
Cohesion: 0.11
Nodes (10): authMode, isAuthenticated, isLoadingHistory, isOnline, isSigningOut, isSyncing, showAuthDialog, showSignOutDialog (+2 more)

### Community 68 - "GradeConfigEntity"
Cohesion: 0.15
Nodes (12): createDefaultPeriods(), emit, form, formRef, handlePeriodTypeChange(), isEditing, isSubmitting, periodType (+4 more)

### Community 70 - "PaymentDaily.vue"
Cohesion: 0.13
Nodes (13): dailyPayments, dialogVisible, emit, { formatCurrency }, formatPaymentMethod(), formatTime(), loading, printDailyReport() (+5 more)

### Community 71 - "professor-form.vue"
Cohesion: 0.17
Nodes (12): applyFilter(), emit, filterForm, grades, loading, resetFilter(), schoolYearOptions, IFilterForm (+4 more)

### Community 72 - "GeneralInfoView.vue"
Cohesion: 0.14
Nodes (13): countries, emit, fileInput, formData, formRef, foundationYears, goBack(), goNext() (+5 more)

### Community 73 - "events.ts"
Cohesion: 0.15
Nodes (9): PreferenceEntity, Column, Entity, PrimaryGeneratedColumn, PreferenceService, Preference, PreferenceCreateInput, PreferenceKey (+1 more)

### Community 74 - "course-form.vue"
Cohesion: 0.17
Nodes (13): closeDialog(), dialogVisible, emit, form, formRef, FormState, grades, handleSubmit() (+5 more)

### Community 75 - "grade-table.vue"
Cohesion: 0.13
Nodes (12): emits, filteredGrades, paginator, props, searchForm, emits, filteredGrades, filteredGradesBase (+4 more)

### Community 76 - "YearRepartionForm.vue"
Cohesion: 0.17
Nodes (6): emit, PaymentDue, paymentDues, props, size, yearRepartition

### Community 77 - "CourseView.vue"
Cohesion: 0.11
Nodes (16): Loader, CourseFormData, courses, loading, newCourseFormRef, updateCourseFormRef, courses, currentCourse (+8 more)

### Community 78 - "DashboardView.vue"
Cohesion: 0.13
Nodes (13): DashboardView(), absenceChartRef, createGradient(), { currency }, currentDate, DashboardStats, loadDashboardStats(), loading (+5 more)

### Community 79 - "PayementConfigurationView.vue"
Cohesion: 0.16
Nodes (13): configurations, { currency }, currentConfig, emit, goBack(), goNext(), isLoading, isSaving (+5 more)

### Community 80 - "loadStudents"
Cohesion: 0.17
Nodes (16): exportToExcel(), exportToPdf(), formatCurrency(), formatCurrencySimple(), getPaymentStatus(), getPaymentStatusLabel(), getPaymentStatusType(), getTuitionDueToDate() (+8 more)

### Community 81 - "Guide de la fonctionnalité de Mise à Jour Automatique"
Cohesion: 0.13
Nodes (14): Changer l'URL du serveur de mises à jour, Configuration, Dépannage, Déploiement en production, Fichiers de configuration, Fonctionnement, Guide de la fonctionnalité de Mise à Jour Automatique, Journaux (+6 more)

### Community 82 - "PreferenceEntity"
Cohesion: 0.21
Nodes (9): IDashboardServiceResponse, IDashboardStats, IRecentAbsence, IRecentPayment, CIVILITY, FAMILY_SITUATION, SCHOOL_TYPE, TEACHING_TYPE (+1 more)

### Community 83 - "SimulationCalculator.vue"
Cohesion: 0.16
Nodes (13): addGrade(), calculateSimulation(), CategoryItem, CategoryResult, categoryResults, ConfigForm, finalAverage, initializeGrades() (+5 more)

### Community 84 - "ProfessorDetailsView.vue"
Cohesion: 0.14
Nodes (8): FAMILY_SITUATION, dialogVisible, loading, loadPhoto(), loadProfessor(), photoUrl, route, router

### Community 85 - "GradeService"
Cohesion: 0.36
Nodes (7): DEFAULT_SCHEDULE_CONFIG, formatMinutesLabel(), formatSlotKey(), formatSlotLabel(), generateSlots(), ScheduleConfigMinutes, loadScheduleConfig()

### Community 86 - "BulletinConfigDialog.vue"
Cohesion: 0.15
Nodes (10): colorPresets, currentTemplateComponent, emit, localColorOptions, Props, sampleGrades, sampleStudent, saveAndClose() (+2 more)

### Community 87 - "CurrencyDisplay.vue"
Cohesion: 0.22
Nodes (7): emit, form, isSubmitting, isValid, Props, requiredReferenceRule, submitForm()

### Community 88 - "PaymentHistoryMini.vue"
Cohesion: 0.14
Nodes (8): adjustedAnnualAmount, annualAmount, loading, paymentData, Props, remainingAmount, totalPaid, IPaymentData

### Community 89 - "course.ts"
Cohesion: 0.16
Nodes (13): ApiResponse, CourseCommand, CourseGroupFormData, CourseGroupResponse, CourseObservation, CourseResponse, IApiResponse, ICourse (+5 more)

### Community 90 - "GradeView.vue"
Cohesion: 0.21
Nodes (11): Grade, GradeType, PaymentAnnualConfig, emit, ExtendedGrade, formData, formRef, goBack() (+3 more)

### Community 91 - "LicenseView.vue"
Cohesion: 0.15
Nodes (11): activateLicense(), ActivateResponse, currentStep, emit, GetMachineIdResponse, isActivating, isCopying, isMachineIdLoading (+3 more)

### Community 92 - "ScheduleConfigEntity"
Cohesion: 0.09
Nodes (16): ScheduleConfigEntity, Column, CreateDateColumn, Entity, JoinColumn, ManyToOne, PrimaryGeneratedColumn, UpdateDateColumn (+8 more)

### Community 94 - "ScheduleService"
Cohesion: 0.06
Nodes (24): GradeType, SyncConfig, SyncHistory, AppConfig, ConfigService, GradeService, ScheduleService, IBranchData (+16 more)

### Community 95 - "license.spec.ts"
Cohesion: 0.15
Nodes (7): DEFAULT_DETAILS_DATA, DEFAULT_STATUS_DATA, handlerRegistry, MockLicenseDetailsData, MockLicenseService, MockLicenseStatusData, rendererInvoke

### Community 96 - "preload.ts"
Cohesion: 0.29
Nodes (10): checkForUpdates(), downloadUpdate(), installUpdate(), invoke(), on(), onDownloadProgress(), onError(), onUpdateAvailable() (+2 more)

### Community 97 - "card.ts"
Cohesion: 0.22
Nodes (7): isFlipped, previewContainer, zoom, ColorScheme, IGradesCentralization, IStudentRank, SchoolInfo

### Community 98 - "ColorSchemeSelector.vue"
Cohesion: 0.17
Nodes (13): customColors, emit, emitCustomColors(), newSchemeName, previewStyle, props, saveCustomScheme(), schemes (+5 more)

### Community 99 - "course-details.vue"
Cohesion: 0.17
Nodes (8): course, dialogVisible, emits, filteredCourses, paginator, props, searchForm, Course

### Community 100 - "phone-verification-list.vue"
Cohesion: 0.18
Nodes (10): emit, formattedPhone, props, toggleVerification(), verificationStatus, emit, phoneVerificationData, props (+2 more)

### Community 101 - "TeachingAssignment.vue"
Cohesion: 0.15
Nodes (10): Course, courses, emit, Grade, grades, props, schoolType, selectedClass (+2 more)

### Community 102 - "student-filter.vue"
Cohesion: 0.18
Nodes (8): dialogVisible, emits, form, formRef, formRule, gradeName, props, BranchCommand

### Community 103 - "themeStore.spec.ts"
Cohesion: 0.22
Nodes (9): APP_THEMES, mockInvoke, AppTheme, CSS_THEME_KEYS, DEFAULT_APP_THEME, THEME_CSS_MAP, ThemeColorKey, colorKeys (+1 more)

### Community 104 - "ClassRoomEntity"
Cohesion: 0.13
Nodes (7): chartRef, currentYear, generalAverage, groupedGrades, periodLabel, props, studentInitials

### Community 105 - "SchoolEntity"
Cohesion: 0.09
Nodes (24): CountryCode, CurrencyCode, SchoolEntity, SchoolSettingsEntity, Column, CreateDateColumn, DeleteDateColumn, Entity (+16 more)

### Community 106 - "gradeService.ts"
Cohesion: 0.18
Nodes (13): IFileData, IFileResponse, IFileServiceParams, IFileServiceResponse, IFileUpload, ReportCardData, ISchoolData, ISchoolServiceParams (+5 more)

### Community 107 - "getAppreciation"
Cohesion: 0.27
Nodes (11): getCategoryAverage(), getCategoryAverage(), getGradeClass(), formatNumber(), getAppreciation(), escapeHtml(), formatBirthDay(), generateBulletinHtml() (+3 more)

### Community 108 - "UpdateNotifier.vue"
Cohesion: 0.20
Nodes (9): DownloadProgress, isUpdateDownloaded, onDownloadProgress(), onUpdateAvailable(), onUpdateDownloaded(), state, UpdateInfo, updateMessage (+1 more)

### Community 109 - "TrancheConfigDetails.vue"
Cohesion: 0.18
Nodes (7): classes, emit, isLoading, props, studentData, updateStudent(), router

### Community 110 - "NewStudentView.vue"
Cohesion: 0.20
Nodes (9): StudentFormInstance, classes, convertSex(), currentSchoolYear, fetchCurrentSchoolYear(), formRef, getCurrentSchoolYear(), handleFileLoaded() (+1 more)

### Community 111 - "student-update.vue"
Cohesion: 0.20
Nodes (3): ./sections/Attachments.vue, imageUrl, props

### Community 112 - "SupervisorInfoView.vue"
Cohesion: 0.18
Nodes (9): { activeIndex, updateActiveIndex }, emit, formData, formRef, goNext(), isResetMode, isSaving, router (+1 more)

### Community 113 - "handlePrint"
Cohesion: 0.21
Nodes (12): calculateClassAverage(), calculateExamAverage(), checkStudentGradesStatus(), findPreviousPeriod(), generateBulletinsHtml(), handlePrint(), loadAnnualData(), loadInitialData() (+4 more)

### Community 114 - "dependencies"
Cohesion: 0.18
Nodes (11): bcryptjs, dotenv, jspdf, dependencies, bcryptjs, dotenv, jspdf, reflect-metadata (+3 more)

### Community 115 - "devDependencies"
Cohesion: 0.18
Nodes (11): cross-env, element-plus, i18n-iso-countries, @iconify-json/ri, devDependencies, cross-env, element-plus, i18n-iso-countries (+3 more)

### Community 116 - "branch-form.vue"
Cohesion: 0.24
Nodes (9): emit, formData, formRef, goBack(), goNext(), isSaving, LanguageFormData, router (+1 more)

### Community 117 - "create-account.vue"
Cohesion: 0.20
Nodes (9): emit, form, formRef, handleSubmit(), initialSchoolInfo, ISchoolData, isSubmitting, rules (+1 more)

### Community 118 - "login-form.vue"
Cohesion: 0.20
Nodes (9): emit, form, formRef, handleLogin(), initialSchoolInfo, ISchoolData, isSubmitting, rules (+1 more)

### Community 119 - "ForgotPasswordView.vue"
Cohesion: 0.18
Nodes (8): formData, formRef, loading, router, rules, securityQuestion, step, submitButtonText

### Community 120 - "course-group-form.vue"
Cohesion: 0.20
Nodes (7): dialogVisible, emits, form, formRef, formRule, props, CourseGroupCommand

### Community 121 - "GradeCalculationDetail.vue"
Cohesion: 0.20
Nodes (8): CalculationDetails, CategoryDetail, classCategories, classGlobalAverage, details, examCategories, GradeDetail, visible

### Community 122 - "./sections/Attachments.vue"
Cohesion: 0.22
Nodes (9): AbsenceEntity, Column, CreateDateColumn, DeleteDateColumn, Entity, JoinColumn, ManyToOne, PrimaryGeneratedColumn (+1 more)

### Community 123 - "SyncHistory.vue"
Cohesion: 0.22
Nodes (5): getTableLabel(), parseErrorTables(), props, sortedHistory, tableLabels

### Community 124 - "ApparenceView.spec.ts"
Cohesion: 0.27
Nodes (10): normSlot(), getClassInfo(), hasSchedule(), checkConflicts(), deleteScheduleItem(), getScheduleItem(), handleDrop(), loadSchedules() (+2 more)

### Community 125 - "ConfigurationWizard.vue"
Cohesion: 0.24
Nodes (8): configViewsKeys, currentStep, currentViewComponent, finishConfiguration(), handleConfigurationSaved(), nextStep(), router, viewComponents

### Community 126 - "LanguageSettingView.vue"
Cohesion: 0.32
Nodes (6): emit, formData, formRef, goBack(), goNext(), rules

### Community 128 - "🔑 License Management"
Cohesion: 0.22
Nodes (8): 🔑 License Management, Private key (vendor only), Recommended IDE Setup, Reminders, School-side activation, Type Support For `.vue` Imports in TS, Vendor CLI, Vue 3 + TypeScript + Vite

### Community 129 - "CardTemplateBase.vue"
Cohesion: 0.22
Nodes (7): cardStyles, currentSchoolYear, GradeInfo, Props, SchoolInfo, Student, validUntil

### Community 130 - "SyncSettings.vue"
Cohesion: 0.28
Nodes (7): emit, handleChange(), intervalOptions, localConfig, props, SyncConfig, SyncHistoryType

### Community 131 - "PlanningScheduleConfig.vue"
Cohesion: 0.14
Nodes (9): classes, ClassItem, endTime, form, lunchEndTime, lunchStartTime, previewSlots, saving (+1 more)

### Community 132 - "User"
Cohesion: 0.25
Nodes (7): Column, CreateDateColumn, DeleteDateColumn, Entity, PrimaryGeneratedColumn, UpdateDateColumn, User

### Community 133 - "License"
Cohesion: 0.32
Nodes (4): License, Column, Entity, PrimaryGeneratedColumn

### Community 134 - "scripts"
Cohesion: 0.25
Nodes (8): scripts, build, dev, dev:debug, license:cli, preview, rebuild:db-driver, test:unit

### Community 135 - "App.vue"
Cohesion: 0.25
Nodes (5): themeStore, app, pinia, router, useThemeStore

### Community 136 - "CardTemplateThree.vue"
Cohesion: 0.25
Nodes (5): cardStyle, currentYear, { isFlipped }, props, validUntil

### Community 137 - "EditableCell.vue"
Cohesion: 0.29
Nodes (5): emit, isNumberType, localValue, onInput(), props

### Community 138 - "student-selector.vue"
Cohesion: 0.32
Nodes (7): emit, emitSelection(), localSelectedStudents, props, selectAllChecked, Student, toggleSelectAll()

### Community 139 - "html2pdf.d.ts"
Cohesion: 0.25
Nodes (4): Html2Pdf, html2pdf.js, html2pdf.js/dist/html2pdf.min, Html2PdfOptions

### Community 140 - "DataLocationView.vue"
Cohesion: 0.06
Nodes (51): FileEntity, Column, CreateDateColumn, DeleteDateColumn, Entity, ManyToOne, PrimaryGeneratedColumn, UpdateDateColumn (+43 more)

### Community 142 - "xlsx"
Cohesion: 0.33
Nodes (5): emits, filteredList, paginator, props, searchText

### Community 143 - "CardTemplateOne.vue"
Cohesion: 0.29
Nodes (4): cardStyle, currentYear, props, validUntil

### Community 144 - "CardTemplateTwo.vue"
Cohesion: 0.29
Nodes (4): cardStyle, currentYear, props, validUntil

### Community 145 - "dashbord-menu.vue"
Cohesion: 0.33
Nodes (4): themeStore, AppItems, MenuItem, HomeView()

### Community 146 - "LicenseChecker.vue"
Cohesion: 0.33
Nodes (6): checkLicense(), daysRemaining, GetStatusResponse, handleLicenseActivated(), LicenseStatus, showLicenseView

### Community 147 - "LoginView.vue"
Cohesion: 0.29
Nodes (5): formData, loading, loginForm, router, rules

### Community 148 - "ApparenceView.vue"
Cohesion: 0.29
Nodes (3): COLOR_FIELDS, currentThemeId, themeStore

### Community 149 - "001_schema_provisioning.sql"
Cohesion: 0.38
Nodes (3): public.create_school_tables(), public.schools_registry, public.sync_history

### Community 150 - "message-customizer.vue"
Cohesion: 0.47
Nodes (5): emit, emitMessageUpdate(), insertPlaceholder(), message, messagePreview

### Community 151 - "MensualityConfigTable.vue"
Cohesion: 0.33
Nodes (5): emits, filteredList, paginator, props, searchText

### Community 153 - "loadStudentGrades"
Cohesion: 0.33
Nodes (6): loadConfig(), loadStudentGrades(), onClassChange(), onPeriodChange(), saveAll(), selectStudent()

### Community 154 - "package.json"
Cohesion: 0.40
Nodes (4): main, name, private, version

### Community 155 - "AbsenceList.vue"
Cohesion: 0.40
Nodes (4): Absence, formattedAbsences, props, Student

### Community 156 - "./sections/SchoolInfo.vue"
Cohesion: 0.50
Nodes (3): emit, goNext(), router

### Community 158 - "preference.ts"
Cohesion: 0.40
Nodes (4): Preference, PreferenceCreateInput, PreferenceKey, PreferenceUpdateInput

### Community 160 - "vacation.ts"
Cohesion: 0.40
Nodes (4): Vacation, VacationCreateInput, VacationStatus, VacationUpdateInput

### Community 161 - "activateLicense"
Cohesion: 0.40
Nodes (5): activateLicense(), closeActivationDialog(), closeGenerateSubDialog(), generateSubLicense(), loadLicenseStatus()

### Community 162 - "WizardViewBase.vue"
Cohesion: 0.25
Nodes (5): handleDelete(), loading, loadProfessors(), professors, router

### Community 163 - "handleAuthSuccess"
Cohesion: 0.70
Nodes (5): checkAuthentication(), handleAuthSuccess(), handleSync(), loadSyncConfig(), loadSyncHistory()

### Community 164 - "components.d.ts"
Cohesion: 0.50
Nodes (3): ComponentCustomProperties, GlobalComponents, vue

### Community 167 - "env.spec.ts"
Cohesion: 0.50
Nodes (3): mockFs, mockPath, IMPORTANT: 'path' is shimmed by vite-plugin-electron-renderer with a CJS

### Community 168 - "Issue with *Red Hat Dependency Analytics Extension*"
Cohesion: 0.50
Nodes (3): Issue with *Red Hat Dependency Analytics Extension*, Problem Statement: the issue is that the project reference itself as a local dependency, so this will lead to recursive building which will end up crashing build task, Solution: Uninstall this extension, delete package-lock.json, node_modules, and install dependencies.

### Community 169 - "selectOptions.ts"
Cohesion: 0.50
Nodes (3): schoolNatures, schoolTypes, studentTypes

### Community 170 - "dashboard.ts"
Cohesion: 0.50
Nodes (3): IDashboardServiceResponse, IDashboardStats, IRecentPayment

### Community 171 - "gradeConfig.ts"
Cohesion: 0.50
Nodes (3): IGradeConfigData, IGradeConfigServiceParams, IGradeConfigServiceResponse

### Community 172 - "homework.ts"
Cohesion: 0.50
Nodes (3): Homework, HomeworkCreateInput, HomeworkUpdateInput

### Community 173 - "scholarship.ts"
Cohesion: 0.50
Nodes (3): Scholarship, ScholarshipCreateInput, ScholarshipUpdateInput

### Community 176 - "calculateCourseAverage"
Cohesion: 0.67
Nodes (3): calculateCourseAverage(), calculateWeightedAverage(), getAverageClass()

### Community 209 - "supabase.ts"
Cohesion: 0.29
Nodes (7): xlsx, exportToExcel(), exportToExcel(), getTotalCoef(), getWeightedAverage(), getWeightedScore(), xlsx

### Community 211 - "pinia"
Cohesion: 0.38
Nodes (3): Homework, HomeworkCreateInput, HomeworkUpdateInput

### Community 212 - "CascadeDelete"
Cohesion: 0.40
Nodes (3): currentDate, processedContent, props

### Community 215 - "./sections/SchoolInfo.vue"
Cohesion: 0.40
Nodes (4): ./sections/SchoolInfo.vue, ClassItem, Props, safeClasses

### Community 233 - "OnboardingView.vue"
Cohesion: 0.50
Nodes (3): activeIndex, updateActiveIndex(), goBack()

### Community 246 - "grade.ts"
Cohesion: 0.40
Nodes (3): currentDate, processedContent, props

## Knowledge Gaps
- **1314 isolated node(s):** `vue`, `GlobalComponents`, `ComponentCustomProperties`, `electronStore`, `SchemaClient` (+1309 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **67 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `PaymentAnnualConfigEntity` connect `grade.ts` to `data-source.ts`, `payment.ts`, `CalculatedGradeEntity`, `DataLocationView.vue`, `xlsx`, `PaymentService`, `main.ts`?**
  _High betweenness centrality (0.087) - this node is a cross-community bridge._
- **Why does `xlsx` connect `supabase.ts` to `ProfessorPaymentView.vue`, `student-table.vue`, `loadStudents`, `devDependencies`, `professor-table.vue`, `student-file.vue`, `PaymentHistory.vue`?**
  _High betweenness centrality (0.086) - this node is a cross-community bridge._
- **Why does `DocumentContentEntity` connect `centralizedPdfService.ts` to `DataLocationView.vue`, `DocumentsView.vue`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Are the 14 inferred relationships involving `GradeEntity` (e.g. with `.addAbsence()` and `.populateEntitySyncMetas()`) actually correct?**
  _`GradeEntity` has 14 INFERRED edges - model-reasoned connections that need verification._
- **Are the 13 inferred relationships involving `CourseEntity` (e.g. with `.populateEntitySyncMetas()` and `.syncCourseGradesDown()`) actually correct?**
  _`CourseEntity` has 13 INFERRED edges - model-reasoned connections that need verification._
- **Are the 10 inferred relationships involving `StudentEntity` (e.g. with `.addAbsence()` and `.populateEntitySyncMetas()`) actually correct?**
  _`StudentEntity` has 10 INFERRED edges - model-reasoned connections that need verification._
- **Are the 9 inferred relationships involving `ProfessorEntity` (e.g. with `.populateEntitySyncMetas()` and `.getTotalProfessors()`) actually correct?**
  _`ProfessorEntity` has 9 INFERRED edges - model-reasoned connections that need verification._