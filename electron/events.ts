...
const global = {
  ...
  backupService: new CloudSyncService(),
...
const convertToResultType = <T>(response: {
  success: boolean;
  data: T | null;
  message: string | null;
  error: string | null;
...
const ensureResultType = (result: any): ResultType => {
    if (!result) {
        return {
            success: false,
            data: null,
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      return handleError(
        error,
        "Erreur lors de la suppression de la salle de classe"
      );
    }
  }
);
...
ipcMain.handle(
  "classRoom:all",
  async (_event: Electron.IpcMainInvokeEvent): Promise<ResultType> => {
    try {
      ...
      return handleError(
        error,
        "Erreur lors de la récupération des salles de classe"
      );
    }
  }
);
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      return handleError(
        error,
        "Erreur lors de l'ajout du cours au groupement"
      );
    }
  }
);
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      return handleError(
        error,
        "Erreur lors de la récupération de tous les cours"
      );
    }
  }
);
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      if (studentData.id) {
        const result = await global.studentService.updateStudent(
          studentData.id,
          studentData
        );
        return {
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      console.log(
        "Données brutes reçues de la base de données:",
      ...
      return handleError(
        error,
        "Erreur lors de la récupération de tous les étudiants"
      );
    }
  }
);
  ...
  async (
    ...
    try {
      ...
      return {
        ...
        data: result.data,
        message:
          result.message || "Récupération des détails de l'étudiant réussie",
        error: result.error,
      };
    } catch (error) {
...
ipcMain.handle(
  "student:downloadDocument",
  async (
    _event: Electron.IpcMainInvokeEvent,
    documentId: number
  ): Promise<ResultType> => {
    try {
      const document = await global.fileService.getFileById({
        fileId: documentId,
      });
      if (!document) {
        return {
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      return handleError(
        error,
        "Erreur lors de la récupération de la photo de l'étudiant"
      );
    }
  }
);
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      return handleError(
        error,
        "Erreur lors de la récupération de la photo du professeur"
      );
    }
  }
);
...
ipcMain.handle("get-class-config", async () => {
  try {
    ...
    console.error(
      "Erreur lors de la récupération de la configuration des classes:",
      error
    );
...
ipcMain.handle("save-class-config", async (_event, config) => {
  try {
    ...
    console.error(
      "Erreur lors de la sauvegarde de la configuration des classes:",
      error
    );
    if (error instanceof Error) {
      return { success: false, error: error.message };
...
ipcMain.handle(
  ...
  async (
    _event: Electron.IpcMainInvokeEvent,
    { studentId, studentData }: { studentId: number; studentData: any }
  ): Promise<ResultType> => {
    console.log(
      "Début de update-student avec les données:",
    ...
    try {
      ...
      if (studentData.document && studentData.document.length > 0) {
        const savedDocuments = await Promise.all(
          studentData.document.map(async (doc: any) => {
            if (doc.content) {
...
ipcMain.handle("absence:allStudent", async () => {
  try {
    ...
    return handleError(
      error,
      "Erreur lors de la récupération de toutes les absences"
    );
  }
});
...
ipcMain.handle(
  "absence:add",
  async (_event, absenceData: IAbsenceServiceParams["addAbsence"]) => {
    try {
...
ipcMain.handle(
  "absence:addProfessor",
  async (_event, data: IAbsenceServiceParams["createProfessorAbsence"]) => {
    try {
...
ipcMain.handle(
  "absence:updateProfessor",
  async (_event, data: IAbsenceServiceParams["updateProfessorAbsence"]) => {
    try {
...
ipcMain.handle("payment:getConfigs", async () => {
  try {
    ...
    return handleError(
      error,
      "Erreur lors de la récupération des configurations de paiement"
    );
  }
});
...
ipcMain.handle("payment:saveConfig", async (_event, configData) => {
  try {
    ...
    return handleError(
      error,
      "Erreur lors de la sauvegarde de la configuration de paiement"
    );
  }
});
...
ipcMain.handle("payment:getByStudent", async (_event, studentId) => {
  try {
    const response = await global.paymentService.getPaymentsByStudent(
      studentId
    );
    return convertToResultType(response);
  } catch (error) {
...
ipcMain.handle("payment:getConfig", async (_event, classId) => {
  try {
    const response = await global.paymentService.getConfigByClass(
      String(classId)
    );
    return convertToResultType(response);
  } catch (error) {
    return handleError(
      error,
      "Erreur lors de la récupération de la configuration de paiement"
    );
  }
});
...
ipcMain.handle("scholarship:getByStudent", async (_, studentId: number) => {
  try {
    const scholarships = await global.scholarshipService.getByStudent(
      studentId
    );
...
ipcMain.handle(
  "scholarship:getActiveByStudent",
  async (_, studentId: number) => {
    try {
      const response = await global.paymentService.getActiveByStudent(
        studentId
      );
      return convertToResultType(response);
    } catch (error) {
...
ipcMain.handle("yearRepartition:getAll", async () => {
  try {
    ...
    return handleError(
      error,
      "Erreur lors de la récupération des répartitions d'année"
    );
  }
});
...
ipcMain.handle("yearRepartition:getCurrent", async () => {
  try {
    const result =
      await global.yearRepartitionService.getCurrentYearRepartition();
    return convertToResultType(result);
  } catch (error) {
    return handleError(
      error,
...
ipcMain.handle("yearRepartition:create", async (_, data) => {
  try {
    const result = await global.yearRepartitionService.createYearRepartition(
      data
    );
    return convertToResultType(result);
  } catch (error) {
    return handleError(
      error,
      "Erreur lors de la création de la répartition d'année"
    );
  }
});
...
ipcMain.handle("yearRepartition:update", async (_, { id, data }) => {
  try {
    ...
    const result = await global.yearRepartitionService.updateYearRepartition(
      id,
      data
    );
    ...
    return handleError(
      error,
      "Erreur lors de la mise à jour de la répartition d'année"
    );
  }
});
...
ipcMain.handle("yearRepartition:delete", async (_, id) => {
  try {
    const result = await global.yearRepartitionService.deleteYearRepartition(
      id
    );
    return convertToResultType(result);
  } catch (error) {
    return handleError(
      error,
      "Erreur lors de la suppression de la répartition d'année"
    );
  }
});
...
ipcMain.handle("yearRepartition:setCurrent", async (_event, id: number) => {
  try {
    ...
    return handleError(
      error,
      "Erreur lors de la mise à jour de la répartition"
    );
  }
});
...
ipcMain.handle(
  "vacation:getByProfessor",
  async (_event, professorId?: number) => {
    try {
...
ipcMain.handle("vacation:update", async (_event, data) => {
  try {
    ...
    if (data && data.id && data.status && Object.keys(data).length <= 3) {
      ...
      return await global.vacationService.updateVacationStatus(
        data.id,
        data.status,
        data.comment
      );
    ...
    if (data && data.id) {
      ...
      console.log(
        "Création d'un nouveau congé avec données mises à jour",
        createData
      );
...
ipcMain.handle(
  ...
  async (_event, { id, status, comment }) => {
    try {
...
ipcMain.handle("gradeConfig:saveGeneral", async (_event, config) => {
  try {
    ...
    return handleError(
      error,
      "Erreur lors de la sauvegarde de la configuration générale"
    );
  }
});
...
ipcMain.handle("school:save", async (_event, schoolData) => {
  try {
    ...
    return handleError(
      error,
      "Erreur lors de la sauvegarde des informations de l'école"
    );
  }
});
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      console.log(
        "Début de professor:create avec les données:",
        JSON.stringify(professorData)
      );
      const response = await global.professorService.createProfessor(
        professorData
      );
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      console.log(
        `Début de professor:getById pour le professeur ID: ${professorId}`
      );
      const response = await global.professorService.getProfessorById(
        professorId
      );
      return {
        success: response.success,
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      console.log(
        `Début de professor:delete pour le professeur ID: ${professorId}`
      );
      const response = await global.professorService.deleteProfessor(
        professorId
      );
      return {
        success: response.success,
...
export function registerReportEvents() {
  ...
  ipcMain.handle(
    "report:generate",
    async (_, data: { studentId: number; period: string }) => {
  ...
  ipcMain.handle(
    ...
    async (_, data: { studentId: number; period: string }) => {
      ...
      try {
        const result = await reportService.getStudentGrades(
          data.studentId,
          data.period
        );
...
ipcMain.handle("professor:payment:update", async (_, paymentData) => {
  try {
    console.log("Données de paiement reçues:", paymentData);
    const result = await global.paymentService.updateProfessorPayment(
      paymentData
    );
...
ipcMain.handle("professor:payment:getById", async (_, paymentId) => {
  try {
    ...
    const result = await global.paymentService.getProfessorPaymentById(
      paymentId
    );
...
ipcMain.handle(
  "file:upload",
  async (_event, fileData: { name: string; type: string; content: string }) => {
    try {
...
ipcMain.handle("absence:createProfessor", async (_, data) => {
  try {
    ...
    return handleError(
      error,
      "Erreur lors de la création de l'absence du professeur"
    );
  }
});
...
ipcMain.handle("file:getUrl", async (_event, filePath) => {
  try {
    ...
    const fileId =
      typeof filePath === "number" ? filePath : filePath?.id || null;
    if (!fileId) {
      return {
        success: false,
        data: null,
...
ipcMain.handle("print:studentCards", async (_event, data) => {
  try {
    ...
    console.error(
      "Erreur lors de la préparation de l'impression des cartes:",
      error
    );
    return {
      success: false,
...
ipcMain.handle(
  ...
  async (_event: Electron.IpcMainInvokeEvent): Promise<ResultType> => {
    try {
      // Obtenir l'utilisateur connecté via le service d'authentification
      const currentUser = await global.authService.getCurrentUser();
      ...
      if (!currentUser?.id) {
        return {
          ...
          error: "NO_USER_CONNECTED",
        };
      }

      const history = await global.backupService.getLocalSyncHistory(
      ...
      return handleError(
        error,
        "Erreur lors de la récupération de l'historique des sauvegardes"
      );
    }
  }
);

// Ajout du handler manquant pour 'backup:config:get'
ipcMain.handle(
  "backup:config:get",
  async (_event: Electron.IpcMainInvokeEvent): Promise<ResultType> => {
    try {
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      await global.backupService.updateSyncConfig(config);
      return {
        success: true,
        data: null,
      ...
      return handleError(
        error,
        "Erreur lors de la mise à jour de la configuration des sauvegardes"
      );
    }
  }
);
  ...
  async (
    ...
    try {
      ...
      return {
        ...
        data: null,
        message: result
          ? "Fichier affiché dans l'explorateur"
          : "Échec de l'affichage du fichier",
        error: result
          ? null
          : "Impossible d'afficher le fichier dans l'explorateur",
      };
    } catch (error) {
      return handleError(
        error,
        "Erreur lors de l'affichage du fichier dans l'explorateur"
      );
    }
  }
);
...
ipcMain.handle(
  ...
  async (_event: Electron.IpcMainInvokeEvent): Promise<ResultType> => {
    try {
      ...
      return handleError(
        error,
        "Erreur lors du marquage du premier lancement comme terminé"
      );
    }
  }
);
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      const result = await global.authService.createSupervisor(
        userData.username,
        userData.password,
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      return handleError(
        error,
        "Erreur lors de la récupération de la question de sécurité"
      );
    }
  }
);
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      const result = await global.authService.validateSupervisor(
        username,
        password
      );
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      const result = await global.authService.validateSecurityAnswer(
        username,
        answer
      );
      console.log("=== Résultat de la validation ===", result);
...
ipcMain.handle("auth:login", async (_event, credentials) => {
    try {
        ...
        return ensureResultType(result);
    } catch (error) {
        return handleError(error, "Erreur lors de la connexion");
    }
});
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      const result = await global.authService.resetPassword(
        username,
        newPassword
      );
    ...
    try {
      ...
      if (
        ...
        return {
          ...
          message:
            "Les champs schoolCode, inspectionZone et departmentCode sont requis",
        };
      }
      ...
      const result = await global.schoolService.saveOrUpdateSettings(
        settingsData
      );
      return {
        success: result.success,
      ...
      return handleError(
        error,
        "Erreur lors de la sauvegarde des paramètres de l'école"
      );
    }
  }
);
      ...
      return {
        ...
        message:
          ...
          (result.success
            ...
            : "Échec de l'activation de la licence"),
    ...
    try {
      ...
      return {
        ...
        message: status.isValid
          ...
          : "La licence est invalide ou absente",
        error: null,
      };
    } catch (error) {
      return handleError(
        error,
        "Erreur lors de la vérification de la validité de la licence"
      );
    }
  }
);

// Ajout des handlers de synchronisation et de test
ipcMain.handle(
  "backup:sync",
  async (): Promise<ResultType> => { 
    try {
      // Étape 1: Récupérer l'utilisateur authentifié depuis le service
      const authUser = await global.backupService.getSupabaseAuthUser();
      if (!authUser) {
        return {
          success: false,
          data: null,
          message: "Utilisateur non authentifié sur le cloud.",
          error: "NOT_AUTHENTICATED",
        };
      }

      // Étape 2: Lancer la synchronisation avec l'ID sécurisé
      const result = await global.backupService.performBidirectionalSync(
        authUser.id
      );
      ...
      return {
        ...
        message: `Synchronisation ${
          result.status === "success" ? "réussie" : result.status
        }`,
        error: result.error_message ?? null,
      };
    } catch (error) {
      return handleError(error, "Erreur lors de la synchronisation");
    }
  }
);

ipcMain.handle("backup:checkConnection", async () => {
  try {
    const isAvailable = await global.backupService.checkSupabaseAvailability();
    return {
      success: true,
      data: { isAvailable },
      message: isAvailable ? "Connexion établie" : "Hors ligne",
      error: null,
    };
  } catch (error) {
    return handleError(error, "Erreur lors de la vérification de la connexion");
  }
});

// Ajouter avec les autres handlers d'authentification
// Ajouter avec les autres handlers d'authentification
ipcMain.handle("auth:checkStatus", async (): Promise<ResultType> => {
    try {
        ...
        const supabaseAvailable = await global.backupService.checkSupabaseAvailability();
        ...
        return {
            ...
            data: {
                ...
                user: currentUser,
                supabaseStatus: {
                    isAvailable: supabaseAvailable,
                    isConnected: await global.authService.isSupabaseSessionValid()
                }
            },
            message: currentUser ? "Utilisateur authentifié" : "Non authentifié",
            error: null
        };
    } catch (error) {
...
ipcMain.handle(
    ...
    async (
        ...
        try {
            ...
            const result = await global.authService.createSupabaseAccount(email, password);
            console.log("=== Résultat création compte ===", result);

            // Si succès, stocker les infos de session
            ...
            return handleError(error, "Erreur lors de la création du compte cloud");
        }
    }
);
...
ipcMain.handle(
    ...
    async (_event: Electron.IpcMainInvokeEvent, 
        ...
        try {
            console.log("=== Connexion compte Supabase ===");
            const result = await global.authService.signInWithSupabase(email, password);
            console.log("=== Résultat connexion ===", result);
            return result;
        } catch (error) {
            console.error("=== Erreur connexion ===", error);
            return handleError(error, "Erreur lors de la connexion au compte cloud");
        }
    }
);
